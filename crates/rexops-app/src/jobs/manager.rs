...
impl JobManager {
    ...
    pub fn poll(&mut self) -> PollOutcome {
        ...
        // Capture the name now, while the mutable borrow of the handle is live —
        // the `push_output` loop below needs `&mut self`, which ends this borrow,
        // and after it the handle would have to be re-read. Cloning the name here
        ...
        if let (Some(exit), true) = (exited, drained) {
            ...
            return PollOutcome {
                repaint: true,
                finished: Some(FinishedJob { outcome, summary }),
            };
        }
