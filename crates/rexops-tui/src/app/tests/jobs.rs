...
fn run_job_to_completion(app: &mut App, name: &str, command: &str) {
    app.jobs.job = Some(spawn(name, name, command, &[]).expect("spawn test job"));
    let deadline = std::time::Instant::now() + std::time::Duration::from_secs(5);
    while app.jobs.job.is_some() {
        app.poll_job();
        assert!(
...
fn finishing_a_job_records_history_and_flashes_a_toast() {
    ...
    assert!(app.jobs.history.is_empty());

    // A clean exit → one Success history entry + a Success toast.
    run_job_to_completion(&mut app, "true", "true");
    assert_eq!(app.jobs.history.len(), 1, "a finished job is recorded");
    let rec = &app.jobs.history[0];
    ...
    assert_eq!(app.jobs.history.len(), 2, "history accumulates");
    let rec = &app.jobs.history[1];
    assert!(!rec.outcome.ok && !rec.outcome.cancelled);
...
fn history_is_capped_and_rolls_off_oldest_first() {
    ...
    for i in 0..JOB_HISTORY_CAP {
        app.jobs.history.push_back(JobRecord {
            name: format!("old-{i}"),
            outcome: LastOutcome {
                name: format!("old-{i}"),
                ok: true,
    ...
    assert_eq!(
        app.jobs.history.len(),
        JOB_HISTORY_CAP,
        "history stays capped"
    );
    assert_eq!(
        app.jobs.history.front().unwrap().name,
        "old-1",
        "the oldest entry rolled off"
    );
    assert_eq!(
        app.jobs.history.back().unwrap().name,
        "newest",
...
fn job_output_is_a_rolling_buffer_capped_at_job_output_cap() {
    ...
    for i in 0..(JOB_OUTPUT_CAP + 250) {
        app.jobs.push_output(JobOutput::Stdout(format!("line-{i}")));
    }
    assert_eq!(app.jobs.output.len(), JOB_OUTPUT_CAP, "buffer stays capped");
    assert_eq!(
        app.jobs.output.front(),
        Some(&JobOutput::Stdout("line-250".to_owned())),
        "the oldest retained line is exactly cap-from-the-end"
    );
    assert_eq!(
        app.jobs.output.back(),
...
fn scroll_starts_at_bottom_and_down_stays_clamped() {
    ...
    assert_eq!(app.jobs.scroll, 0, "default follows the bottom");
    // Down (newer) at the bottom is a no-op, never underflows.
    app.jobs.scroll(false);
    assert_eq!(app.jobs.scroll, 0);
}
...
fn scroll_up_clamps_to_the_buffer() {
    ...
    for _ in 0..100 {
        app.jobs.scroll(true);
    }
    assert_eq!(app.jobs.scroll, 4, "up clamps at len-1 (5 lines)");
...
fn starting_a_job_resets_scroll_to_the_bottom() {
    ...
    assert_eq!(app.jobs.scroll, 0, "a fresh job follows the bottom again");
    if let Some(job) = app.jobs.job.as_mut() {
        job.cancel();
    }
}
...
fn up_down_scroll_the_jobs_screen_via_on_action() {
    ...
    app.on_action(Action::Up, &mut runner);
    assert_eq!(app.jobs.scroll, 2, "Up scrolls toward older output");
    app.on_action(Action::Down, &mut runner);
    assert_eq!(app.jobs.scroll, 1, "Down scrolls back toward newest");
}
...
fn poll_job_reports_change_when_a_job_finishes() {
    ...
    let mut app = bare_app();
    app.jobs.job = Some(spawn("false", "false", "false", &[]).expect("spawn test job"));
    ...
    assert_eq!(app.jobs.history.len(), 1, "completion was recorded");
}
